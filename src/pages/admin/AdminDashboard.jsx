...
export default function AdminDashboard() {
  ...
  const recentRequests = [...requests].reverse().slice(0, 4);
  ...
  return (
    <div className="flex min-h-screen bg-[#FAF7F2]">
      ...
      <main className="flex-1 ml-72 p-10">
        <DashboardHeader onOpenReport={() => setReportOpen(true)} />
        ...
        <div className="grid lg:grid-cols-2 gap-6 mt-8">
          ...
          <RecentActivity requests={requests} users={users} volunteers={volunteers} />
        </div>
        ...
        <section className="grid xl:grid-cols-3 gap-8 mt-10">
          <div className="bridge-card h-full xl:col-span-2">
            <div className="flex justify-between items-center mb-8">
              ...
              <Link to="/admin/requests" className="flex items-center gap-2 text-[#D08C60] font-medium">
                View All
                <ArrowRight size={18} />
              </Link>
            </div>
            ...
            <div className="space-y-4">
              {recentRequests.map((request) => (
                <div key={request.id} className="flex justify-between items-center rounded-xl border bg-white px-5 py-4">
                  <div>
          ...
          <div className="bridge-card h-full">
            ...
            <div className="space-y-4">
              <button onClick={() => navigate("/admin/users")} className="w-full flex items-center gap-4 rounded-xl bg-[#FAF1EB] p-5 text-left">
              ...
              <button onClick={() => navigate("/admin/requests")} className="w-full flex items-center gap-4 rounded-xl bg-[#F4F8F5] p-5 text-left">
              ...
              <button onClick={() => navigate("/admin/volunteers")} className="w-full flex items-center gap-4 rounded-xl bg-[#EEF5FF] p-5 text-left">
                <BadgeCheck className="text-[#D08C60]" />
                <div>
                  <h3 className="font-semibold">Manage Volunteers</h3>
                  <p className="text-sm text-gray-500">Monitor volunteer participation.</p>
                </div>
              </button>

              <button onClick={() => setReportOpen(true)} className="w-full flex items-center gap-4 rounded-xl bg-[#FFF7E8] p-5 text-left">
                <FileBarChart2 className="text-[#6B8F71]" />
                <div>
                  <h3 className="font-semibold">Dashboard Reports</h3>
                  <p className="text-sm text-gray-500">Generate platform reports and summaries.</p>
                </div>
              </button>
            </div>
          </div>
        ...
        <section className="grid lg:grid-cols-2 gap-8 mt-10">
          ...
          <PlatformInsights requests={requests} volunteers={volunteers} />
        </section>
